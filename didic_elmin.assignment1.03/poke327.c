...
static void dijkstra_distance_map(map_t *m)
{
  ...
  while ((p = heap_remove_min(&h))) {
    p->hn = NULL;
      ...
      //Top left
      if ((path[p->pos[dim_y] - 1][p->pos[dim_x] - 1].hn) && (path[p->pos[dim_y] - 1][p->pos[dim_x] - 1].cost > (p->cost + m->weight[p->pos[dim_y] - 1][p->pos[dim_x] - 1]))) {
        path[p->pos[dim_y] - 1][p->pos[dim_x] - 1].cost = ((p->cost + m->weight[p->pos[dim_y] - 1][p->pos[dim_x] - 1]));
      ...
      //Top middle
    if ((path[p->pos[dim_y] - 1][p->pos[dim_x]    ].hn) && (path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost > (p->cost + m->weight[p->pos[dim_y] - 1][p->pos[dim_x]]))) {
        path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost = ((p->cost + m->weight[p->pos[dim_y] - 1][p->pos[dim_x]]));
    ...
    //Left
    if ((path[p->pos[dim_y]    ][p->pos[dim_x] - 1].hn) && (path[p->pos[dim_y]    ][p->pos[dim_x] - 1].cost > (p->cost + m->weight[p->pos[dim_y]][p->pos[dim_x] - 1]))) {
        path[p->pos[dim_y]][p->pos[dim_x] - 1].cost = ((p->cost + m->weight[p->pos[dim_y]][p->pos[dim_x] - 1]));
...
static int place_character(map_t *m) {
    while(1) {
        player_x = (rand() % ((MAP_X - 1))) + 1;
        player_y = (rand() % ((MAP_Y - 1))) + 1;
        if(m->map[player_y][player_x] == ter_path) {
            m->map[player_y][player_x] = player_p;
            break;
        }
    }
...
static int set_weight_hiker(map_t *m) {
    int x,y;
    ...
    for(y = 0; y < MAP_Y; y++) {
        for(x = 0; x < MAP_X; x++) {
            ...
            else if(m->map[y][x] == ter_grass) {
                m->weight[y][x] = 15;
            }
            else if(m->map[y][x] == ter_clearing) {
                m->weight[y][x] = 10;
            }
            else if(m->map[y][x] == ter_forest) {
                m->weight[y][x] = 15;
            }
            else if(m->map[y][x] == ter_mountain) {
                m->weight[y][x] = 15;
            }
            else if(m->map[y][x] == player_p) {
                m->weight[y][x] = 0;
            }
            else {
                m->weight[y][x] = 999;
...
static int set_weight_rival(map_t *m) {
    int x,y;
...
int main(int argc, char *argv[])
{
  ...
  set_weight_rival(world.cur_map);
  dijkstra_distance_map(world.cur_map);
  set_weight_hiker(world.cur_map);
  dijkstra_distance_map(world.cur_map);
